#This code has been modified by Ryan Burner rburner@usgs.gov
#from code was originally created by Jacob Levine and Perry de Valpine
...
#Set working directory (should work for Windows RStudio users)
setwd(
  dirname(
    rstudioapi::callFun(
...
#mac/linux users may will need to do this manually
# setwd()
...
# So we all get same data set
set.seed(1)                 
# Number of sites
nSites <- 100  
# Number of presence/absence measurements (visits/surveys) per site
nVisits <- 2    
# Matrix to contain the obs. data
...
##Is A PSI ESTIMATE CALCULATED THIS WAY BIASED IN A PARTICULAR DIRECTION? WHAT ABOUT p?
  #Type your thoughts here:
    # https://docs.google.com/spreadsheets/d/1Y4tEk2GLsOg7lml3WE01XJZ0Rm_ldx7ykpRpdPX_NW0/edit?usp=sharing 
...
# 'data' are observed quantities (responses or predictors) in nimble
nim.data <- list(y = y)
nim.data
# 'constants' are indexing quantities in nimble
nim.constants <- list(nSites = nrow(y), nVisits = ncol(y));nim.constants
nim.constants
...
Section10p3_code <- nimbleCode({
  ...
  for (i in 1:nSites) {    # Loop over sites
    z[i] ~ dbern(psi)         # State/process model (the [model-estimated] truth)
    for (j in 1:nVisits) { # Loop over replicate surveys
      y[i,j] ~ dbern(z[i] * p)  # Observation model (the detection * truth)
    }
  }
})
...
# Ensure that z starts at '1' whenever there were 1 or more '1' observations
zst <- apply(y, 1, max)
zst
# Initial values are also supplied as a list of named initial values
#start p and psi at a moderate value
inits <- list(z = zst,p=0.5,psi=0.5)
inits

# Parameters to be monitored (for mcmc to store results)
#can also store "z" if you want - total occupancy often of interest
#I leave out z here to make output easier to visualize
params <- c("psi", "p") 
...
# (Takes ~3 min on my standard Windows laptop)
fm2 <- nimbleMCMC(
  code = Section10p3_code, # model object
  data = nim.data,  # data values
  constants = nim.constants,    # constants - index values and fixed values
...
# Then simulate data and run model (with appropriate p, Psi, nVisits)...
#using the code above, but modifying parameters for simulation
#
...
wt <- read.csv("./data/willowtits.csv",comment="#")
...
#this shows how many sites have 1, 2, or 3 surveys
table( n <- rowSums(!is.na(Y)) ) 
#NAs as missing data are fine in nimble
#[if you want NAs to be estimated, include prior definition in model - we won't]

# Extract and standardize the site covariates
elevS <- standardize(wt$elev) #elevation
#elevation^2 included to allow e.g., hump-shaped relationship
...
#again, to allow quadratic relationships
day2=day^2
...
#(zero is the ~mean for standardized covariates)
#{NAs are ok for response variable, but generally not for covariates}
dayS[is.na(dayS)] <- 0
day2S[is.na(day2S)] <- 0
durS[is.na(durS)] <- 0

#some technical details on priors...ignore for now if confusing 
...
#note the addition (unlike above) of occupancy and detection covariates
#note that here we use 'b' (beta) for occupancy intercept / slopes and
#'a' (alpha) for detection intercept / slopes
#that is arbitrary and a bit unconventional
wt_surveyCovs.code <- nimbleCode({
  
  # Priors  ### ADD FOREST BETA PRIOR HERE
  ...
  # Likelihood
  for(i in 1:nSites) { # for i sites
    # occupancy (process) model  # ADD FOREST COVER AND COVARIATE HERE
    logit(psi[i]) <- b0 + #intercept
                          #forest cover 
      (bElev * ele[i]) +  #elevation
      (bElev2 * ele2[i]) #elevation squared
    
    #here we estimate occupancy (z) at each site
    z[i] ~ dbern(psi[i])  
    
    # observation (detection) model
    #still inside the site loop, note additional 'visit' loop for detection model
    for(j in 1:n[i]) { # for j visits to each site
      logit(p[i, j]) <- a0 + #intercept
        (aDur * dur[i, j]) + #duration
        (aDay * day[i, j]) +  #day
        (aDay2 * day2[i, j]) #day squared
      
      #here we estimate detection in each survey
...
# Prepare input data for NIMBLE
#sum detections
ysum <- rowSums(Y, na.rm=TRUE)
ysum
#create initial values for z
#(all sites with any detection must start at 1)
z <- ifelse(ysum > 0, 1, NA)
# we won't bother with initial values for alphas, betas, just z
# nimble will draw one from priors

#format data
nim.data <- list(Y = Y, z = z) 
str(nim.data)
...
#parameters to monitor ### ADD FOREST BETA COVARIATE HERE ###
params <- c("p0", "a0", "aDur", "aDay", "aDay2",
            "psi0", "b0",      "bElev", "bElev2", "N") #z also if you want  
...
# Call nimble and summarize posteriors
# This compiles (to C++, for speed) and runs the model all in one step
#this takes ~4 min on my average Windows laptop
...
# Do a plot with credible intervals 
#(here we will only use the first chain for simplicity)
...
#perhaps because of better sampling during this time?
hist(day)
...
# ========================
#download forest and elevation raster map of Switzerland (included in package)
data(Switzerland)
str(Switzerland)
