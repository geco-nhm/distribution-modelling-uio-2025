# BIOS 5211/9211
# Olav Skarpaas, Eva Lieungh, Peter Horvath, Adam Naas and Ingrid V. Tjessem,
# NHM, University of Oslo, Nov 2021, 
# with updates by AN Nov 2023 and OS and IVT 2025
...
# Make a presentation (about 15 min in total) of individual results
# (2-3 min per person), as well as summary across the group.

# Species list (one per person):
# ------------------------------
# Fagus sylvatica, a tree
# Dracocephalum ruyschiana, a plant
# Platismatia norvegica, a lichen
# Canis lupus, a carnivore
# Alces alces, a herbivore
# Uria aalge, a bird
...
training.data.Norway <- generate.training.data(sp,predictors.Norway,generate.absences=TRUE,absence.value=NA,n.abs=1000,factors=c("ar50.artype.2km","geo.norge123.2km"))   # To see how the function 'generate.training.data' works, open the file 'Dataprep.functions.r'
...
# Modify and repeat the following for all four grains:

# Load environmental predictor data as rasters
r.list <- list.files("lab_data/RTPD/TIFF_GRAIN/100m", pattern="\\.tif$", full.names=TRUE)
predictors.100m <- rast(r.list)

# Generate training data for your species for the particular grain
training.data.100m <- generate.training.data(sp,predictors.100m,generate.absences=TRUE,absence.value=NA,n.abs=1000,factors=c("ar50.artype","geo.norge123"))   # To see how the function 'generate.training.data' works, open the file 'Dataprep.functions.r'
...
training.data.100pst <- generate.training.data(sp.100pst,predictors,generate.absences=TRUE,absence.value=NA,n.abs=1000,factors=c("ar50.artype","geo.norge123"))   # To see how the function 'generate.training.data' works, open the file 'Dataprep.functions.r'
