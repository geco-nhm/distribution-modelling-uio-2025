...
# Updates (raster to terra package) by OS, Nov 2025
...
# factors: names of variables (character vector) to be treated as factors (or unique parts of the variable names)
{
  ...
  occ_points <- data.frame(lon=sp.data$decimalLongitude,lat=sp.data$decimalLatitude)
  occ_points <- vect(occ_points,crs="+proj=longlat +datum=WGS84")
  occ_UTM33 <- project(occ_points,crs(predictors))
  ...
  presences <- which(terra::values(occ_ras)==1)
  ...
  if(generate.absences)
  {
    ...
    # such as maxent).
    absences <- which(terra::values(occ_ras)==0)
  ...
  # Extract predictor data at location of presences and absences (i.e., selected cells)
  xy <- xyFromCell(occ_ras,cell=selected)
  training_data <- data.frame(presence=terra::values(occ_ras)[selected],
                              xy,
